...
public class ElementFactoryGenerator : IIncrementalGenerator
{
    ...
    private static readonly DiagnosticDescriptor UnsupportedGenericElementClass = new(
        id: "DMX004",
        title: "Generic Datamodel element class cannot register its schema",
        messageFormat: "Generic element class '{0}' must be a top-level partial class without a static constructor, so that each constructed type can register its schema; until then its properties are not stored as attributes",
        category: "Datamodel",
        defaultSeverity: DiagnosticSeverity.Warning,
        isEnabledByDefault: true,
        description: "The ElementFactory only lists closed types, and the type arguments of a generic class are only known where it is constructed. The generator gives a partial generic class a static constructor that registers the schema of each constructed type before its first instance is created.");
    ...
    private static void Execute(SourceProductionContext context, (Compilation Left, ImmutableArray<INamedTypeSymbol?> Right) tuple)
    {
        ...
        var genericClasses = new List<ElementClass>();
        ...
        foreach (var type in elementTypes.OrderBy(type => type.ToDisplayString()))
        {
            ...
            {
                // an abstract generic class is never constructed, its properties are bound by each subclass
                if (type.IsAbstract)
                {
                    continue;
                }

                if (type.ContainingType is not null || !IsPartial(type) || type.StaticConstructors.Any(ctor => !ctor.IsImplicitlyDeclared))
                {
                    context.ReportDiagnostic(Diagnostic.Create(UnsupportedGenericElementClass, type.Locations.FirstOrDefault() ?? Location.None, type.ToDisplayString()));
                    continue;
                }

                genericClasses.Add(new ElementClass(type));
                continue;
            }
        ...
        {
            var hintName = genericClass.Type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat.WithGlobalNamespaceStyle(SymbolDisplayGlobalNamespaceStyle.Omitted).WithGenericsOptions(SymbolDisplayGenericsOptions.None));
            context.AddSource($"{hintName}_{genericClass.Type.Arity}.ElementSchema.g.cs", emitter.EmitGenericSchema(genericClass));
        }
    }

    private static bool IsPartial(INamedTypeSymbol type)
    {
        return type.DeclaringSyntaxReferences.All(reference => reference.GetSyntax() is ClassDeclarationSyntax declaration && declaration.Modifiers.Any(SyntaxKind.PartialKeyword));
    }
    ...
    private sealed class Emitter(SourceProductionContext context, Compilation compilation, List<ElementClass> classes)
    {
        ...
        public string Emit()
        {
            ...
            {
                source.AppendLine();
                EmitSection(source, type, SectionNames[type]);
            }
        ...
        /// <summary>
        /// Writes the other half of a partial generic class: a static constructor that registers the schema of the constructed type,
        /// followed by one section per class in its inheritance chain, nested in the class so that they are constructed with its type arguments.
        /// </summary>
        public string EmitGenericSchema(ElementClass genericClass)
        {
            var type = genericClass.Type;
            var chain = genericClass.Chain.ToList();
            var version = LibraryVersion(compilation.GetTypeByMetadataName(ElementTypeName)?.ContainingAssembly);
            ...
            source.Append($$"""
                ...
                // Generated by KeyValues2.ElementFactoryGenerator for Datamodel.NET {{version}}.
                //
                // Registers the schema of every type constructed from {{type.ToDisplayString()}} before its first instance is created,
                // since the ElementFactory of this assembly can only list closed types.
                #nullable enable
                #pragma warning disable

                using System.Runtime.CompilerServices;
                using PropertyBinding = global::Datamodel.PropertyBinding;
            ...
            var groups = string.Concat(chain.Select((_, index) => $", DatamodelSection{index}.Properties"));

            source.AppendLine($"    partial class {type.Name}<{string.Join(", ", type.TypeParameters.Select(parameter => parameter.Name))}>");
            source.AppendLine("    {");
            source.AppendLine($"        static {type.Name}()");
            source.AppendLine("        {");
            source.AppendLine($"            global::Datamodel.ElementSchema.Register(new global::Datamodel.ElementSchema(typeof({TypeName(type)}), {Literal(type.Name)}{groups}));");
            source.AppendLine("        }");

            for (var index = 0; index < chain.Count; index++)
            {
                source.AppendLine();
                EmitSection(source, chain[index], $"DatamodelSection{index}");
            }
        ...
        /// </summary>
        void EmitSection(StringBuilder source, INamedTypeSymbol type, string sectionName)
        {
            ...
            source.AppendLine($"        static class {sectionName}");
            source.AppendLine("        {");
